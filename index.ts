...
type BaseColumnOptions<Name extends string, T> = {
	name: Name
	__nullable?: boolean
	__default?: T
}
...
interface Column<
	Name extends string,
	T,
	options extends BaseColumnOptions<Name, T> = {
		name: Name
	},
> {
	options: BaseColumnOptions<Name, T>
	nullable: () => Column<Name, T, Clean<options & { __nullable: true }>>
	default: <V extends T>(value: V) => Column<Name, T, Clean<options & { __default: V }>>
}
...
>(name: Name, defaultOptions?: options): Column<Name, T, options> {
	const options = {
		name,
		...(defaultOptions ?? {}),
	}
...
type LiteralName = string | number | boolean

type LiteralColumn<T extends LiteralName> = Column<"literal", T>

function literal<const T extends LiteralName>(value: T): LiteralColumn<T> {
	const base = Column<`literal`, T, { name: "literal"; __literalValue: T }>("literal", {
		name: "literal",
...
console.log(something.options.name)
