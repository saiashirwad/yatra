...
type ResolveAlias<T extends string> =
  T extends AliasName ? AliasMap[T] : T
...
type ParseEntry<E, S extends DbScopeSchema> =
  E extends string ? ParseFieldDefinition<E, S>["type"]
  : E extends object ?
    Resolve<
      {
...
type ValidateFieldDefinition<S, T extends string> =
  T extends `${infer Type}.${infer Modifiers}` ?
    `${ValidateType<S, Type>}${ValidateModifiers<S, Modifiers, Type>}`
  : ValidateType<S, T>

type ValidateType<S, T extends string> =
  T extends PossibleFieldTypes<S> ? T
  : {
    ...
    Suggestion extends string ?
      Suggestion
    : `Invalid field type '${T}'. Did you mean one of: ${PossibleFieldTypes<S> & string}`
  : never

type ExtractModel<Type extends string> =
  Type extends `${infer Model}[]` ? Model : Type
...
> =
  Mods extends "" ? ""
  : Mods extends (
    `${infer Mod extends string}.${infer Rest extends string}` // Add constraints here
  ) ?
...
> =
  Mod extends PossibleModifiers ?
    ...
    : Mod
  : {
    [K in PossibleModifiers]: K extends `${Mod}${string}` ?
      K
    : never
