...
>(
  ...paths: [
    ...{
      [K in keyof Paths]: Conform<
        Paths[K],
        string & ValidatePath<T, Paths[K]>
      >
    }
  ]
) {
...
export type GetField<T extends Tableish, Key> =
  Key extends keyof TableishFields<T> ?
...
export type GetPath<T extends Tableish, Path> =
...
> =
  ...
  : Path extends keyof TableishFields<T> ?
    `${Prefix}${Path}`
  : {
      [k in keyof TableishFields<T>]: k extends (
        `${Path}${string}`
      ) ?
        `${Prefix}${k}`
      : never
